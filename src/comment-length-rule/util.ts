...
/**
 * @todo incorrect? in vscode the line is at the ruler but the visual length is over the ruler. a
 * tab does not count as its full width if part of it overlaps with other characters. so i either
 * need to support that or maybe use a config setting to have two strategies?
 *
 * A tab character moves the cursor to the next tab stop. If part of the way there already, then
 * that is not
 */
export function vislen(string: string, tabWidth = 2) {
  console.log('input str len:', string.length);

  if (string.length === 0) {
    return 0;
  }

  // let's say tab width is 2 and we have the string 'a\t\t'. the correct length of this string is
  // 4, not 5, because this tab advanced to the second tab stop.

  const tabs = string.match(/\t/g);
  const tabCount = tabs ? tabs.length : 0;

  console.log('tab count:', tabCount);

  const nonTabCharCount = string.length - tabCount;

  console.log('non tab char count:', nonTabCharCount);

  // just feeling it out, this is very wrong.

  const ceil = roundUpToNearestMultiple(nonTabCharCount, tabWidth);
  console.log('ceil:', ceil);

  console.log('remainder:', ceil / tabCount);

  const output = ceil / tabCount * tabWidth;

  console.debug('visual length of text "%s" is %d', string.replace(/\t/g, '\\t'),
    output);

  return output;

  // return string.length - tabCount + tabCount * tabWidth;
}

/**
 * Adapted from https://stackoverflow.com/questions/3407012
 */
export function roundUpToNearestMultiple(number: number, multiple: number) {
